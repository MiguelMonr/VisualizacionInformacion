...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(column(5,(leafletOutput("mapaCarla", height="600px"))),
...
server <- function(input, output){
    ...
    output$mapaCarla <- renderLeaflet({
      if(input$var == "Por población") {

      leaflet(mxpolig) %>%
        addTiles() %>%
        addPolygons(group="Por Población",
                    stroke = F,
                    smoothFactor = 0.2,
                    opacity=1.0,
                    fillOpacity = 0.5,
        ...
        addLegend(group="Por Población",position="bottomleft", pal=palPob,
